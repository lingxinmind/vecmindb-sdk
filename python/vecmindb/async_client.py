...
class AsyncVecminClient:
    ...
    async def create_tenant(self, name: str, **kw) -> Dict[str, Any]:
        ...
        """
        data = await self._api_post("/tenants", {"name": name}, **kw)
        res_data = data.get("data") if isinstance(data, dict) else None
        return res_data if isinstance(res_data, dict) else data

    async def list_tenants(self, **kw) -> List[CollectionInfo]:
        ...
        """
        return await self.list_collections(**kw)
    ...
    async def ensure_collection(
        ...
        *,
        ...
        Args:
            ...
            domain: Cognitive factuality domain.
            sovereignty_token: Sovereign domain access token forwarded to create_collection.
