...
class AsyncVecminClient:
    ...
    async def mcp_store_memory(
        ...
        **kw,
        ...
        This is a convenience wrapper that calls ``POST /api/v1/mcp/message``
        with a properly formatted JSON-RPC ``tools/call`` payload.
        ...
        data = await self._api_post("/mcp/message", payload, **kw)
        result = data.get("data", data)
        if isinstance(result, dict):
    ...
    async def mcp_search_memory(
        ...
        **kw,
        ...
        Returns:
            Search result text from the MCP server.
        """
        payload = {
            "jsonrpc": "2.0",
            "method": "tools/call",
